...
#Must call begin first time this is run
    ...
    def store_old(self):
        ...
        statement = str(self.user.tempAPI.temp) + ',' + str(self.user.tempAPI.cloud_cover) + ',' + str(self.user.tempAPI.time)
        f.write(str(statement))
        f.close()

    def startUpdate(self):
        print 'In Get Values'
        print 'writing to', self.file_name
        self.user.tempAPI.refresh()
        self.new_panel[0] = self.user.tempAPI.temp + self.user.weather_offset
        self.new_panel[1] = self.user.tempAPI.cloud_cover
        self.new_panel[2] = self.user.tempAPI.time
        ...
        f.close()
        if(len(old_data) > 1):
            print 'reading old data: ', old_data[0]
            self.old_panel[0] = float(old_data[0]) + self.user.weather_offset
            self.old_panel[1] = float(old_data[1])
            self.old_panel[2] = float(old_data[2])
        else:
            self.old_panel[0] = self.user.tempAPI.temp + self.user.weather_offset
            self.old_panel[1] = self.user.tempAPI.cloud_cover
            self.old_panel[2] = self.user.tempAPI.time
        self.store_old()
        self.fade_LED()
